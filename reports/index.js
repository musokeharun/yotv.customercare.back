...
class Report {
  ...
  totalAndRange = async () => {
    ...
    return prisma.$queryRaw(
      `SELECT COUNT(id) as value,createdAt as label ${
        step ? "," + step("createdAt") + " AS other" : ""
      } FROM ${"`Call`"} WHERE date(createdAt) BETWEEN date("${from}") AND date("${to}") GROUP BY date(createdAt),${
        step ? step("createdAt") : `date(createdAt)`
      };`
    );
  };
